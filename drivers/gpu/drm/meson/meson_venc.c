			      ...
			      struct drm_display_mode *mode)
{
	...
	if (!vmode) {
		pr_err("%s: Fatal Error, unsupported vic %d\n",
			__func__, vic);
		return;
	}
	...
	if (use_enci) {
		...
		writel(vmode->enci.sch_adjust, priv->io_base + _REG(ENCI_VIDEO_SCH));
