...
static const struct dw_hdmi_phy_ops meson_dw_hdmi_phy_ops = {
	...
	.phy_disable = dw_hdmi_phy_disable,
};
				...
				void *data)
{
	...
	/* Setup HPD Filter */
	dw_hdmi_top_write(meson_dw_hdmi, HDMITX_TOP_HPD_FILTER,
			  (0xa << 12) | 0xa0);

	/* Enable HPD and HDMI-TX Interrupt */
	dw_hdmi_top_write(meson_dw_hdmi, HDMITX_TOP_INTR_STAT_CLR, 0x1f);
	...
	dw_plat_data->read_hpd = dw_hdmi_read_hpd;
